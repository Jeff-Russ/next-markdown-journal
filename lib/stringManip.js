...
;``
export function deleteAll(str, patterns) {
  let result = str
  for (let pattern of patterns)
    if (pattern instanceof RegExp) result = result.replace(pattern, '')
    else result = result.replaceAll(pattern, '')
  return result
}

export function replaceEach(str, findReplacePairs) {
  let result = str
  findReplacePairs.forEach(([find, replace]) => {
    if (find instanceof RegExp) result = result.replace(find, replace)
    else result = result.replaceAll(find, replace)
  })
  return result
}
